...
This sub-repository is concerned with the problem of forming linear $Y \approx
A X + b + \epsilon$ (where $\epsilon$ is a zero-mean Gaussian with covariance
$Q$) approximations to general statistical models $p(y \mid x)$ which either
exhibit additive noise:
...
This approximation is done by minimizing (approximately for the latter, exactly
for the former) the expected [Kullback-Leibler divergence](https://en.wikipedia.org/wiki/Kullback%E2%80%93Leibler_divergence)
