...
def inverse_cdf(sorted_uniforms: ArrayLike, logits: ArrayLike) -> Array:
    ...
    1. The CPU implementation is a numba-compiled specialized searchsorted(arr, vals) which is guaranteed to run in
        O(N + M) where N is the size of logits and M that of sorted_uniforms. This could be replaced mutatis mutandis
        by np.searchsorted (not jnp.searchsorted!) but the latter does not guarantee this execution time.
    2. On GPU, we use searchsorted with a sorting strategy: this is proceeds by merge (arg) sorting, which works in log(N+M) 
        and is efficient for large arrays of sorted uniforms, typically our setting.
